...
%
%ARJN, how about how to calculate the wavevector from a vector sum? i.e. something similar to fig 2, augmented with ki, kf.
\begin{equation}
...
%ARJN, we've not said what a scattering length density is. Do we want to do that here? I think it's worth it. We need to define it for N and X.
% The scattering length density of a material is calculated by summing over all the scattering lengths of all the different types of atoms in a material and dividing by the molecular volume (which itself can be calculated from the mass density of a material),
%\begin{equation}
%    \rho_m = \sum_{i} \frac{b_i}{V_m}
%    \label{equ:sld}
%\end{equation}
% For neutrons $b_i$ are the coherent scattering lengths of a material, and are tabulated at <TODO>. For X-rays the scattering length of an atom is energy dependent <REF>, but to a first approximation can be calculated as $b_i = z_i \dot r_e$, where $z_i$ is the atomic number and $r_e$ is the classical electron radius, $2.817 \times 10^{-5} \AA$.
